...
function setupEventListeners() {
  ...
  document
    .getElementById("finishBattleBtn")
    .addEventListener("click", finishBattle);
}
...
function updatePlayersList(players) {
  ...
  if (players.length === 0) {
    container.innerHTML =
      '<p class="text-glow-purple text-center">No players online</p>';
    return;
  }
        ...
        <div class="flex items-center justify-between p-3 bg-gray-800 rounded border border-purple-500">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-sm text-glow-purple">Level ${
                      player.level
                    } • Rating: ${player.rating || 1200}</div>
                </div>
            </div>
            <button onclick="challengePlayer('${
              player.id
            }')" class="retro-btn retro-btn-pink text-sm">
                CHALLENGE
            </button>
        </div>
    `
...
async function startBattle(data) {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/battle/${data.battleId}/questions`,
      {
        headers: { Authorization: `Bearer ${AUTH_TOKEN}` },
      }
    );
  ...
  document.getElementById("player1Level").textContent =
    battleState.player1.level;
  document.getElementById("player2Name").textContent =
    battleState.player2.username;
  document.getElementById("player2Level").textContent =
    battleState.player2.level;
...
function generateMockQuestions() {
  return [
    ...
    },
    {
      ...
      description:
        "Which algorithm solves maximum subarray problem efficiently?",
      options: [
        "Brute force",
        "Kadane's algorithm",
        "Divide and conquer",
...
async function finishBattle() {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/battle/${battleState.battleId}/finish`,
      {
        method: "POST",
        headers: {
    ...
    showResults({
      winner:
        battleState.score > battleState.opponentScore
          ? battleState.player1
          : battleState.player2,
      player1: {
        ...battleState.player1,
      ...
      performance: {
        accuracy: Math.round(
          (battleState.answers.filter((a) => a.correct).length /
            battleState.answers.length) *
            100
        ),
...
function showResults(results) {
  ...
  document.getElementById(
    "resultPlayer1Avatar"
  ).src = `https://api.dicebear.com/7.x/avataaars/svg?seed=${results.player1.username}`;
  document.getElementById("resultPlayer1Name").textContent =
    results.player1.username;
  document.getElementById("resultPlayer1Score").textContent =
    results.player1.score + " pts";
  document.getElementById("resultPlayer1Time").textContent =
    results.player1.time + "s";

  // Player 2
  document.getElementById(
    "resultPlayer2Avatar"
  ).src = `https://api.dicebear.com/7.x/avataaars/svg?seed=${results.player2.username}`;
  document.getElementById("resultPlayer2Name").textContent =
    results.player2.username;
  document.getElementById("resultPlayer2Score").textContent =
    results.player2.score + " pts";
  document.getElementById("resultPlayer2Time").textContent =
    results.player2.time + "s";

  // Performance
  document.getElementById("performanceDetails").innerHTML = `
        <div class="grid grid-cols-2 gap-4">
            <div><span class="text-glow-purple">Accuracy:</span> <span class="text-glow-pink">${
              results.performance.accuracy
            }%</span></div>
            <div><span class="text-glow-purple">Avg Time:</span> <span class="text-glow-pink">${
              results.performance.avgTime
            }s</span></div>
            <div><span class="text-glow-purple">Correct:</span> <span class="text-glow-pink">${
              results.performance.correctAnswers
            }/${results.performance.totalQuestions}</span></div>
            <div><span class="text-glow-purple">Rating Change:</span> <span class="text-glow-pink">${
              isWinner ? "+25" : "-15"
            }</span></div>
        </div>
    `;
